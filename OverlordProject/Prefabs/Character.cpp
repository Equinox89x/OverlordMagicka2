...
void Character::Update(const SceneContext& sceneContext)
{
	...
	if (m_pCameraComponent->IsActive())
	{
		...
		if (sceneContext.pInput->IsMouseButton(InputState::down, 1))
		{
			...
			look.x = static_cast<float>(movement.x);
			look.y = -static_cast<float>(movement.y);
		}
		...
		auto transform{ m_pCameraComponent->GetGameObject()->GetTransform() };

		//Retrieve the forward & right vector (as XMVECTOR) from the TransformComponent
		XMVECTOR forward{ XMLoadFloat3(&transform->GetForward()) * move.y };
		XMVECTOR right{ XMLoadFloat3(&transform->GetRight()) * move.x };
		...
		m_MoveAcceleration += m_MoveAcceleration * deltaTime;
		...
		XMStoreFloat3(&displacement, XMLoadFloat3(&m_TotalVelocity) * deltaTime);
		m_pControllerComponent->Move(displacement, epsilon);
