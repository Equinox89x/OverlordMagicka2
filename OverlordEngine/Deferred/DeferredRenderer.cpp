...
void DeferredRenderer::Initialize()
{
	...
	m_GBuffer[int(eGBufferId::Diffuse)] = CreateRenderTarget(w,h,DXGI_FORMAT_B8G8R8A8_UNORM);
	m_GBuffer[int(eGBufferId::Specular)] = CreateRenderTarget(w, h, DXGI_FORMAT_B8G8R8A8_UNORM);
...
void DeferredRenderer::Begin(const SceneContext& sceneContext) const
{
	...
	//pDeviceContext->ClearDepthStencilView(m_pDefaultDepthStencilView, D3D11_CLEAR_DEPTH, 1.f, 0.f);
...
void DeferredRenderer::End(const SceneContext& sceneContext) const
{
	...
	//...
	ID3D11ShaderResourceView* pSRV[RT_COUNT - 1] = { nullptr };
