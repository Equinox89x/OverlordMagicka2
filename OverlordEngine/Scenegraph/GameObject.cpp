...
#include "../../OverlordProject/Prefabs/EnemyRangedCharacter.h"
#include "../../OverlordProject/Prefabs/MagickaCharacter.h"
...
{
	for (auto pOtherObject : m_EnemiesInRange)
	{
		if (auto enemy{ dynamic_cast<EnemyMeleeCharacter*>(pOtherObject) }) {
			m_EnemiesInRange.remove(enemy);
			break;
		}
		if (auto enemy2{ dynamic_cast<EnemyRangedCharacter*>(pOtherObject) }) {
...
void GameObject::OnTrigger(GameObject* pTriggerObject, GameObject* pOtherObject, PxTriggerAction action)
{
	...
	{
		if (pOtherObject->GetMarkedForDestroy()) return;
		if (auto enemy{ dynamic_cast<EnemyMeleeCharacter*>(pOtherObject) }) {
		...
		if (auto enemy3{ dynamic_cast<MagickaCharacter*>(pOtherObject) }) {
